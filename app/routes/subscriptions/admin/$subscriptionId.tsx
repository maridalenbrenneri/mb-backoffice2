...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (_action === 'update') return await updateAction(values);

  if (_action === 'update-internal-note') {
    return await updateInternalNote(values);
  }

  if (_action === 'update-special-request') {
    return await updateSpecialRequest(values);
  }
...
export default function UpdateSubscription() {
  ...
  useEffect(() => {
    ...
    setEditDisabled(!!loadedSubscription.wooSubscriptionId);
  }, [loadedSubscription]);
    ...
    <main>
      ...
      >
        ...
        <Typography variant="h1">
          Subscription: <small>{subscription.recipientName}</small>
        </Typography>
            ...
            <Paper sx={{ p: 2 }}>
              {subscription.wooSubscriptionId && (
                <Alert severity="info" sx={{ m: 1 }}>
                  This subscription is imported, update of status, customer data
                  and quantities must be done in Woo Admin.
                </Alert>
              )}
              <Form method="post">
                ...
                <Box my={2}>
                  {renderStatus(subscription.status, editDisabled)}
                  {renderFrequency(subscription.frequency, editDisabled)}
                  {renderShippingTypes(subscription.shippingType, editDisabled)}
                </Box>
                <Box>
                  <FormControl>
                    <QuantityTextField
