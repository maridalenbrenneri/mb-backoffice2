...
import { getNextTuesday } from '~/_libs/core/utils/dates';
...
export class RoastService {
  private static calculateWeightByCoffee(_250: any, _500: any, _1200: any) {
    const coffee1kg =
  ...
  private static _agg(quantity: number) {
  ...
  private static fromItems(orders: OrderEntity[]) {
    const data = new Map();
  ...
  private static aggregateCoffeesOrders(
    orders: OrderEntity[],
    _250: any,
    _500: any,
    _1200: any
  ...
  private static aggregateCoffeesFromSubscriptions(
    ...
    subscriptions.forEach((s) => {
      let agg = this._agg(s.quantity250);
      _250.coffee1 += agg.coffee1;
      _250.coffee2 += agg.coffee2;
      _250.coffee3 += agg.coffee3;
      _250.coffee4 += agg.coffee4;

      agg = this._agg(s.quantity500);
      _500.coffee1 += agg.coffee1;
      _500.coffee2 += agg.coffee2;
      _500.coffee3 += agg.coffee3;
      _500.coffee4 += agg.coffee4;

      agg = this._agg(s.quantity1200);
      _1200.coffee1 += agg.coffee1;
      _1200.coffee2 += agg.coffee2;
      _1200.coffee3 += agg.coffee3;
      _1200.coffee4 += agg.coffee4;
  ...
  private static resolveCoffee(coffees: ProductEntity[], productId: number) {
    console.debug('resolveCoffee', productId);
    return coffees.find((c) => c.id === productId);
  }

  static getRoastOverview(
    subscriptions: SubscriptionEntity[],
    ...
    if (delivery.type === 'MONTHLY') {
      ...
      const aggSubscriptions = this.aggregateCoffeesFromSubscriptions(
    ...
    if (nonRecurringOrders.length) {
      this.aggregateCoffeesOrders(nonRecurringOrders, _250, _500, _1200);
    ...
    if (customOrders.length) {
      const map = this.fromItems(customOrders);
      ...
      if (
        delivery.product2.id &&
        !list.some((l) => l === delivery.product2.id)
      ) {
      ...
      for (const [key, value] of map.entries()) {
        if (
          ...
          const coffee = this.resolveCoffee(coffees, key);
    ...
    const weight = this.calculateWeightByCoffee(_250, _500, _1200);
